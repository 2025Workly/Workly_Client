...
export default function Calculator() {
  ...
  const lookupTax = (taxable: number, familyCount: number): number => {
    if (taxable <= 1000000) return 0;
    if (taxable <= 2000000) return 15000;
    if (taxable <= 3000000) return 30000;
    return 50000 + (taxable - 3000000) * 0.05;
  };

  const calcTax = () => {
    const taxableIncome = money - taxFree;
    const baseTax = lookupTax(taxableIncome, numFamily);

    let childDeduction = 0;
    if (numKids === 1) childDeduction = 12500;
    else if (numKids === 2) childDeduction = 29160;
    else if (numKids >= 3) childDeduction = 29160 + (numKids - 2) * 25000;

    const incomeTax = Math.max(baseTax - childDeduction, 0);
    ...
    const totalTax =
      nationalPension + health + care + jobInsurance + incomeTax + localTax;
  ...
  const plusMinusBtnList = [
    {
      title: "20세 이하 자녀 수",
      setNumPlus: () => setNumKids(numKids + 1),
      setNumMinus: clickMinusBtnCh,
      numValue: numKids,
    },
    {
      title: "부양 가족 수(본인포함)",
      setNumPlus: () => setNumFamily(numFamily + 1),
      setNumMinus: clickMinusBtn,
      numValue: numFamily,
    },
  ];
  ...
  return (
    <div className={styles.allContainer}>
      ...
      <div className={styles.inputCalcContainer}>
        ...
        <div
          style={{ display: "flex", gap: "304px", margin: "104px 0 140px 0" }}
        >
          <div>
            <PlusMinusBtn
      ...
      <div className={styles.resultContainer} ref={resultRef}>
        ...
        <div className={styles.resultList}>
          ...
          <div className={styles.result}>
            ...
            <div style={{ display: "flex" }}>
              <p className={styles.FinalresultWon}>
                {formatComma(result.realTakeHome)}
              </p>
              <p className={styles.FinalresultWon} style={{ fontSize: "26px" }}>
                원
              </p>
            </div>
          </div>
